...
// Swapped Strapi import for the main Sanity products query
import { getProducts } from "@/sanity/lib/queries";
import { MoveRight, Sparkles } from "lucide-react";
import Reveal from "../ui/Reveal";
import RecentProductsGrid from "./RecentProductsGrid";

// Define strict Sanity interfaces to satisfy ESLint
...
const RecentProducts = async () => {
  // Fetch the 4 newest products from Sanity
  const products = await getProducts(4); 

  // Map Sanity's string IDs to numeric IDs to prevent prop errors in RecentProductsGrid/Card
  const mappedProducts = products?.map((p: SanityProduct) => ({
    ...p,
    id: p._id as unknown as number,
    badges: p.badges?.map((b: SanityBadge) => ({ ...b, id: b._id as unknown as number })),
    variants: p.variants?.map((v: SanityVariant) => ({ ...v, id: v._key as unknown as number }))
  })) || [];
    ...
    <section className="relative bg-zinc-50 py-24 sm:py-32 overflow-hidden border-t border-zinc-200">
      ...
      <div className="relative mx-auto max-w-400 px-6 lg:px-12">
        ...
        {mappedProducts.length > 0 ? (
            <RecentProductsGrid products={mappedProducts} />
        ) : (
          <Reveal>
