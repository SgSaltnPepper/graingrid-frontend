...
export default function Navbar() {
  ...
  // State for expanding categories (Accordion logic)
  ...
  useEffect(() => { 
      setOpen(false); 
      setShowMega(false); 
      setExpandedCatId(null); // Reset accordion on navigation
  }, [pathname]);
                ...
                {categories.map((cat) => {
                   ...
                   return (
                    <div key={cat.documentId || cat.id} className="border-b border-zinc-50 pb-2">
                        {/* Parent Category Link or Toggle */}
                        <div className="flex items-center justify-between group cursor-pointer" onClick={() => hasSubs && toggleAccordion(cat.id)}>
                            ...
                            {hasSubs && (
                                <ChevronDown 
                                    size={20} 
                                    className={`text-zinc-400 transition-transform duration-300 ${isExpanded ? "rotate-180 text-orange-600" : ""}`} 
                                />
                            )}
                        </div>

                        {/* Animated Subcategories List */}
                        {hasSubs && (
                            <div className={`overflow-hidden transition-[max-height] duration-500 ease-in-out ${isExpanded ? "max-h-60" : "max-h-0"}`}>
                                <div className="flex flex-col gap-3 pl-4 pt-3">
                                    {/* Subcategories only (Removed the "All" link) */}
                ...
                {categories.map((cat) => {
                    ...
                    return (
                        <div key={cat.id} className="flex flex-col">
                            ...
                            <div 
                                className="flex items-center justify-between text-xl font-black uppercase tracking-widest text-zinc-300 hover:text-white transition-colors cursor-pointer group"
                                onClick={() => {
                                    if(hasSubs) toggleAccordion(cat.id);
                                    else window.location.href = `/products?category=${cat.Name}`;
                                }}
                            >
                                <span>{cat.Name}</span>
                                {hasSubs ? (
                                    <ChevronDown size={20} className={`text-zinc-500 transition-transform duration-300 ${isExpanded ? "rotate-180 text-orange-500" : ""}`} />
                                ) : (
